...
fastify.post('/api/tasks', async (request, reply) => {
    const { title, description, reward, assigneeId, isGlobal, creatorTgId } = request.body;
    const creator = await prisma.user.findUnique({ where: { telegramId: creatorTgId } });
    if (!creator || creator.role !== 'parent')
        return reply.status(403).send({ error: 'Only parents can create tasks' });
...
fastify.put('/api/tasks/:id', async (request, reply) => {
    const { id } = request.params;
    const { title, description, reward, assigneeId, isGlobal } = request.body;
    return prisma.task.update({
        where: { id: Number(id) },
...
fastify.post('/api/tasks/:id/verify', async (request, reply) => {
    const { id } = request.params;
    const { approve, parentTgId } = request.body;
    ...
    if (task.status !== 'pending')
        return reply.status(400).send({ error: 'Task not in pending state' });
    if (approve) {
        // Find who completed it (the assignee)
        if (!task.assigneeId)
            return reply.status(400).send({ error: 'No assignee for verification' });
...
fastify.post('/api/shop', async (request, reply) => {
    const { title, description, price, creatorTgId } = request.body;
    const creator = await prisma.user.findUnique({ where: { telegramId: creatorTgId } });
    if (!creator || creator.role !== 'parent')
        return reply.status(403).send({ error: 'Unauthorized' });
...
fastify.post('/api/shop/buy', async (request, reply) => {
    const { itemId, userTgId } = request.body;
    const user = await prisma.user.findUnique({ where: { telegramId: userTgId } });
    const item = await prisma.storeItem.findUnique({ where: { id: itemId } });
    if (!user || !item)
        return reply.status(404).send({ error: 'User or Item not found' });
